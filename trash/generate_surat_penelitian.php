...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($nama) && !empty($NIM) && !empty($judul)) {
        ...
        $content = "
        Adalah mahasiswa Jurusan Teknik Informatika, Fakultas Teknik, Universitas Jenderal Soedirman dan dinyatakan
        ";
        ...
        $pdf->SetXY(72, $yBebas);
        $pdf->MultiCell(112, 10, 'telah selesai melaksanakan penelitian tugas akhir di');
        $pdf->SetXY(25, $yBebas+5);
        $pdf->MultiCell(90, 10, 'Laboratorium Jurusan Teknik Informatika ');
        
        // Kembalikan font ke ukuran normal
        $pdf->SetFont('times', '', 12);
        
        $content = "Fakultas Teknik Universitas Jenderal ";
        $pdf->SetXY(113, $yBebas+5);
        $pdf->MultiCell(75, 10, $content);
        $content = "Soedirman. Demikian surat ini dibuat untuk dapat digunakan sebagaimana mestinya.";
        ...
        $pdf->SetXY(130, $pdf->GetY()); // Atur posisi tanda tangan kanan
        ...
        $pdf->SetXY(70, $pdf->GetY() + 30); // Atur posisi nama tanda tangan kiri
        $pdf->MultiCell(80, 40, $ttdKiriNama, 0, 'L', false, 0, '', '', true, 0, false, true, 50, 'T');

        // Set posisi nama tanda tangan kanan agar sejajar dengan nama tanda tangan kiri
        $pdf->SetXY(130, $pdf->GetY()); // Atur posisi nama tanda tangan kanan
        ...
        $pdf->SetFont('times', '', 11);
        $pdf->SetXY(70, $pdf->GetY() + 5); // Atur posisi nip tanda tangan kiri
        $pdf->MultiCell(80, 40, $ttdKiriNip, 0, 'L', false, 0, '', '', true, 0, false, true, 50, 'T');
        
        // Set posisi nip tanda tangan kanan agar sejajar dengan nip tanda tangan kiri
        $pdf->SetXY(130, $pdf->GetY()); // Atur posisi nip tanda tangan kanan
